...
func createAccessToken(email string, duration int64) (string, error) {
	...
	claims := jwt.MapClaims{
		"email": email,
		"iss":   config.Env.ASYNQMON_BASE_URL,
		"exp":   duration,
	}

	token := jwt.NewWithClaims(signingMethod, claims)
	tokenString, err := token.SignedString([]byte(config.Env.ACCESS_TOKEN_SECRET_KEY))
	if err != nil {
...
func loginHandler(res http.ResponseWriter, req *http.Request) {
	...
	_, err = firebaseAuth.VerifyIDToken(ctx, body.IDToken)
	if err != nil {
